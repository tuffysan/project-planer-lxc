...
[[ -n "$STORAGE" ]] || { echo "FEL: Kunde inte hitta storage med rootdir. Ange STORAGE=..."; exit 1; }
...
[[ -n "$TEMPLATE_STORAGE" ]] || { echo "FEL: Kunde inte hitta storage för templates."; exit 1; }
...
if [[ -z "$TEMPLATE" ]]; then
  ...
  [[ -n "$TEMPLATE_NAME" ]] || { echo "FEL: Debian 12 template hittades inte."; exit 1; }
...
cleanup_failed_container() {
  local rc=$?
  if [[ $rc -ne 0 && "$CONTAINER_CREATED" == "1" ]]; then
    echo
...
[[ "$network_ok" == "1" ]] || { echo "FEL: Nätverket blev inte tillgängligt i containern."; exit 1; }
